...
export class CSInterface {
  ...
  getSystemPath(pathType: string): string {
    let path = decodeURI((<any>window).__adobe_cep__.getSystemPath(pathType));
  ...
  getHostCapabilities(): any {
    const hostCapabilities = JSON.parse((<any>window).__adobe_cep__.getHostCapabilities());
  ...
  getExtensions(extensionIds: string[]): any {
    ...
    const extensionsStr = (<any>window).__adobe_cep__.getExtensions(extensionIdsStr);
  ...
  getNetworkPreferences(): any {
    const result = (<any>window).__adobe_cep__.getNetworkPreferences();
  ...
  initResourceBundle(): any {
    const resourceBundle = JSON.parse((<any>window).__adobe_cep__.initResourceBundle());
  ...
  getCurrentApiVersion(): ApiVersion {
    const apiVersion = JSON.parse((<any>window).__adobe_cep__.getCurrentApiVersion());
    return apiVersion;
  };
   ...
   * </Menu>
   */
  setContextMenu(menu: string, callback: (clikedMenuId: string) => unknown) {
    if ("string" != typeof menu) {
      return;
    }
   ...
   *
   */
  setContextMenuByJSON(menu: string, callback: (clikedMenuId: string) => unknown) {
    if ("string" != typeof menu) {
      return;
    }
